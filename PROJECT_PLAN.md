...
> 计划状态：阶段 A 已完成，阶段 B、C、D、E 代码候选已完成并已本地合并到 `dev`，阶段 B/C/D 待用户原生验收，阶段 E 已完成用户原生验收，阶段 F `0.3.38` 已完成自动门禁、本地合并和用户原生验收，阶段 G `0.3.39` 已完成代码实现、开发侧门禁、本地 NSIS 候选、本地快进合并和用户原生验收，阶段 H `0.3.40` 已完成代码、门禁、本地 NSIS、用户原生验收和发布
> 编制日期：2026-09-04
> 当前发布基线：v0.3.40
> 下一候选版本：待规划
...
- 自动门禁、本地 `0.3.39` NSIS 候选、独立分支快进合并和 Windows 11/Tauri/WebView2 原生验收均已完成；`0.3.39` 仍未 push、未创建 Tag 或 Release。
...
- 阶段 H 完成前，本地 `0.3.40` 安装包只能称为本地候选，必须等待用户在 Windows 11/Tauri/WebView2 环境执行最终矩阵。
...
- `v0.3.40` Tag、`dev`/`main` 远端同步和 GitHub Release 按用户授权完成；发布后文档冻结，不再修改。

### 12.8 当前执行状态

- 2026-09-05，用户确认阶段 H 的 Windows 11/Tauri/WebView2 原生验收已完成；阶段 H 验收状态标记为已完成。用户未提供具体 Windows、WebView2、显示器或样本明细，本文不补填这些环境字段。
- 阶段 H 的五个版本入口已统一为 `0.3.40`；本地 NSIS 候选已重新构建并通过 loader 校验，具体构建证据记录在 `PROJECT_PROGRESS.md`。
- `v0.3.40` 发布文档已冻结；Release 包含 Windows x64 NSIS 安装包和仅含主程序与 `WebView2Loader.dll` 的便携 ZIP，并明确 WebView2 Runtime、LibreOffice、签名状态和预览限制。
