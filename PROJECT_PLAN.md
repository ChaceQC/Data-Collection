...
> 计划状态：执行中
> 编制日期：2026-09-01
> 当前工作分支：codex/implement-phases-a-b
> 当前发布基线：0.3.16
> 下一阶段版本：0.3.17
...
- 阶段开发以 dev 为基线，可使用 `codex/` 前缀的短生命周期分支；本轮阶段 A-B 使用 `codex/implement-phases-a-b`。
...
- 已删除旧版总体计划内容并建立本文件。
- 阶段 A `0.3.17` 和阶段 B `0.3.18` 已完成代码候选、最小自动验证和浏览器回退检查。

当前未完成：

- 阶段 C 及后续阶段尚未实现；当前版本入口为阶段 B 的 `0.3.18` 代码候选，正式发布基线仍为 `v0.3.16`。
- 阶段 A-B 的 Windows 11/Tauri/WebView2 原生窗口、真实索引刷新和多 DPI 验收仍需单独执行。
- 阶段 C-J 仍是待执行阶段，不应在代码尚未完成时提前修改版本或宣传为已发布；阶段 A-B 的 `0.3.18` 仍只是代码候选。

下一步：

1. 按阶段 C 增加预览失败后的重试、定位、复制位置和连续浏览入口。
2. 为阶段 C 补齐预览失败状态模型、契约测试、构建和浏览器回退检查。
3. 阶段 C 代码门禁通过后，将五个版本入口更新为 0.3.19，并单独记录 Windows 手工验收状态。
