...
> 计划状态：阶段 F Windows 11/Tauri/WebView2 原生验收、文档收口和 v0.3.32 发布已完成
> 编制日期：2026-09-02
> 当前工作分支：dev
> 当前发布基线：v0.3.32
> 计划候选版本：v0.3.32（已发布）
...
| 阶段 | 目标版本 | 主题 | 交付重点 | 状态 |
| --- | --- | --- | --- | --- |
| A | 0.3.27 | 文件库数据契约 | 新查询 command、返回投影、IPC 校验、文件库模型 | 代码、自动验证和阶段 F 原生验收已完成 |
| B | 0.3.28 | 悬浮球外观和面板骨架 | 球体状态视觉、数量徽标、文件库标题、面板尺寸和稳定布局 | 代码、自动验证和阶段 F 原生验收已完成 |
| C | 0.3.29 | 文件列表交互 | 全量列表、搜索、筛选、排序、分页、加载和空状态 | 代码、自动验证和阶段 F 原生验收已完成 |
| D | 0.3.30 | 主窗口连续工作流 | 文件定位、文件夹目录、资源管理器显示和直接预览 | 代码、自动验证、浏览器回退和阶段 F 原生验收已完成 |
| E | 0.3.31 | 同步和稳定性 | 拖放反馈、revision 同步、竞态、混合 DPI、窄窗口和性能 | 代码、自动验证、浏览器回退、NSIS 候选和阶段 F 原生验收已完成 |
| F | 0.3.32 | 完整验收和候选发布 | 文档收口、自动验证、Windows 11 验收、安装包候选 | 原生验收、文档收口、版本入口同步和 v0.3.32 发布已完成 |

每个阶段必须先完成代码和最小验证，再更新该阶段版本；阶段 F 验收完成后已将五个版本入口统一同步为 `0.3.32`，并按授权完成 Tag、远端同步和 Release 发布。
...
- [x] 将普通状态图标调整为文件库/文件堆语义，保留现有图标库，不手写重复 SVG。
- [x] 增加当前文件库数量徽标，数量加载中、读取失败、超过上限时分别有稳定表现。
- [x] 设计普通、悬停、拖入、记录中、记录成功、部分成功和失败状态的颜色、图标和外环。
- [x] 成功和失败标记不遮挡球体主图标，且在窗口边缘、缩放和混合 DPI 下仍在球体范围内。
- [x] 控制动画时长和触发频率，加入 `prefers-reduced-motion` 降级。
- [x] 保证 `64 x 64 DIP` 命中区域、拖拽手势、键盘焦点和 tooltip 不发生回归。

### 7.4 实现清单：面板骨架

- [x] 将标题从“最近记录”改为“文件库”或“全部资料”，移除 `recent` 语义命名。
- [x] 将数量显示从 `n / 5` 改为总条目数，并处理加载中和读取失败状态。
- [x] 将默认面板尺寸调整为约 `360 x 420 DIP`，由几何模型在窄工作区中压缩到安全最小值。
- [x] 增加标题区、搜索区、筛选区、列表占位区和反馈区的固定结构。
- [x] 为列表行定义稳定的最小高度、图标尺寸、文本截断和右侧动作空间。
- [x] 增加加载骨架、空列表、空搜索结果和查询错误的视觉占位，不依赖内容高度改变窗口尺寸。
- [x] 保持面板左右方向、球体锚点和窗口工作区约束不变。
- [x] 为文件库面板建立独立 class 命名，避免把主窗口表格样式直接复制进悬浮窗。

### 7.5 验证

- [x] 在普通、悬停、拖入、记录中、成功和失败状态下检查图标、颜色、徽标和提示一致。
- [x] 浏览器演示模式已检查文件库标题、数量徽标和固定行布局；加载骨架、空列表、空搜索结果和查询错误分支已落地为稳定组件状态。
- [x] 在 1280px、680px、360px 视口检查面板不溢出、文本不遮挡、列表仍可滚动。
- [x] 检查左侧和右侧展开、四角位置、工作区不足时的面板压缩。
- [x] 运行 `npm.cmd run test:floating-ball` 和 `npm.cmd run build`。
- [x] 运行几何模型、悬停状态机和窗口尺寸相关的最小 Rust/前端检查。

### 7.6 阶段版本门禁

- [x] 同步五个版本入口到 `0.3.28`。
- [x] 在 `PROJECT_PROGRESS.md` 记录外观、面板尺寸、浏览器截图检查和待进行的 Windows 原生窗口验收。
- [x] 执行 `git diff --check`，确认没有加入构建产物、截图临时文件或真实资料。
- [x] 提交阶段变更后进入阶段 C。
...
- [x] 将面板主数据从 `recent` 调整为 `files` 或等价文件库语义，禁止继续通过 `getRecentEntries` 限制主列表。
- [x] 初次打开面板时读取文件库，显示加载中、成功、空列表、无搜索结果和错误重试状态。
- [x] 实现文件名和元数据搜索；查询输入采用受控长度，并使用轻量 debounce。
- [x] 实现“全部、收藏、文件夹、失效”筛选，并显示当前筛选状态。
- [x] 实现名称、类型、修改时间和最近打开时间排序，默认名称升序。
- [x] 实现受控分页；列表使用悬浮窗内部滚动，不带入主窗口滚动条。
- [x] 显示文件/文件夹图标、名称、类型、大小、分组摘要和失效状态。
- [x] 收藏按钮保留现有行为，收藏更新后不清除搜索、筛选、排序和滚动上下文。
- [x] 空列表提供打开主窗口资料库入口；空搜索结果提供清除搜索入口。
- [x] 浏览器回退使用多条无敏感信息的演示文件项，覆盖文件、文件夹、收藏、失效、同名、空格名和长文件名。
- [x] 拖放成功后刷新当前文件库；新条目若不符合当前筛选，不强行插入当前视图。

### 8.4 验证

- [x] 模型测试覆盖 1/5/6、50 和 51 条边界；浏览器演示覆盖 11 条并验证列表内部滚动。
- [x] `get_floating_files` 从索引快照读取全部登记条目，包含只由主窗口导入、从未通过悬浮球拖入的资料。
- [x] 最近没有拖放记录但索引已有资料时，文件库使用独立全量查询结果而仍有内容。
- [x] 浏览器检查覆盖搜索、清空、筛选、排序方向、刷新后的列表、数量和空状态；查询模型覆盖分页结果一致性。
- [x] 长中文文件名、空格文件名、同名文件、文件夹和失效路径在默认及 360px 演示视口没有横向溢出。
- [x] 浏览器检查收藏成功状态；代码路径保留收藏忙碌锁、失败提示和查询上下文，Windows 原生失败注入待用户验收。
- [x] 运行 `npm.cmd run test:floating-ball`、文件库模型测试和 `npm.cmd run test:contracts`。
- [x] 运行 `npm.cmd run build`，确认 Sites 产物仍然生成。

### 8.5 阶段版本门禁

- [x] 同步五个版本入口到 `0.3.29`。
- [x] 在 `PROJECT_PROGRESS.md` 记录文件库列表、搜索、筛选、排序和分页验证结果。
- [x] 同步 `README.md` 和 `prototype/README.md` 中关于悬浮球“最近记录”的描述，明确区分当前已实现和下一阶段未完成的快捷操作。
- [x] 提交阶段变更后进入阶段 D。
...
- [x] 文件行主点击调用 `open_main_from_floating(fileId)`，主窗口显示、获得焦点并接收 `floating-open-file` 事件。
- [x] 主窗口收到文件 ID 后，切换到包含该条目的有效列表上下文，清除会阻塞目标的搜索/筛选条件。
- [x] 主窗口选中对应文件并计算目标页、滚动到可见位置，确保“定位到该文件”不是只打开一个不可见的预览层。
- [x] 点击文件夹后，主窗口进入该索引文件夹的目录视图，更新面包屑并显示目录子项。
- [x] 文件夹不存在、权限不足或路径失效时，不进入空目录；显示明确错误、重试或返回资料库入口。
- [x] 文件和文件夹的点击行为使用同一个安全 ID 入口，由主窗口根据索引条目的 `kind` 区分处理。
- [x] 操作成功后文件定位/预览会收起悬浮面板，资源管理器定位保留面板并显示结果反馈，不留下失焦的透明窗口。

### 9.4 实现清单：资源管理器显示

- [x] 文件行增加“在资源管理器中显示”图标，提供明确的 tooltip 和可访问名称。
- [x] 操作调用已有 `reveal_indexed_file(fileId)` 安全 command，不向前端暴露任意路径拼接能力。
- [x] 文件和文件夹都覆盖资源管理器定位；文件夹定位到文件夹本身，文件定位到文件或其父目录的既有约定。
- [x] 路径失效、权限拒绝、资源管理器启动失败时显示明确错误，并保留主窗口反馈入口。
- [x] 快捷操作的点击事件阻止行点击冒泡，避免同时触发预览、定位和资源管理器三个动作。
- [x] 该操作保持外部程序启动边界：只在用户明确点击时执行，不在悬浮窗打开或刷新时静默启动资源管理器。

### 9.5 实现清单：直接预览

- [x] 文件行增加直接预览入口，支持 `.md`、`.txt`、图片、视频、`.docx`、`.xlsx`、`.doc`、`.pdf` 等已有登记格式的统一预览入口。
- [x] 直接预览入口通过主窗口已有预览会话和资源协议启动，不在悬浮窗复制格式判断、文件读取或渲染实现。
- [x] 支持预览的文件从悬浮窗触发后，主窗口显示、获得焦点、定位条目并进入既有 `loading` 到 `ready` 的预览流程。
- [x] 不支持、损坏、加密、过大、权限不足、缺少 DOC 转换器和路径失效的情况复用已有明确状态，不显示空白预览。
- [x] 预览入口在不支持时可以打开主窗口并显示错误详情和默认程序打开/资源管理器定位等下一步，不直接静默失败。
- [x] 预览打开后继续支持关闭、相邻浏览、收藏、重试和资源释放，悬浮窗关闭不持有 preview resource。
- [x] 文件行主点击使用 `locate` 意图，直接预览使用同一 `floating-open-file` 事件的 `preview` 意图，避免两套行为不一致。
...
- [x] 运行预览、契约、资料库和新增快捷操作模型测试；当前结果分别为 17 项、16 项、23 项通过。
- [x] Rust 侧运行 `cargo fmt --check`、`cargo check --locked` 和 `cargo test --locked`，当前结果为 77 项通过。

### 9.7 阶段版本门禁

- [x] 同步五个版本入口到 `0.3.30`。
- [x] 在 `PROJECT_PROGRESS.md` 逐条记录四项快捷工作流的自动验证和 Windows 原生验收状态。
- [x] 同步 README 中悬浮球能力描述，明确浏览器回退或模型测试不等同于真实资源管理器/预览验收。
- [x] 提交阶段变更后进入阶段 E。
...
- [x] `index-changed`、`floating-recorded`、收藏变化、重命名、重新定位和索引移除都能刷新文件库当前上下文。
- [x] 刷新时保留搜索词、筛选、排序、当前页和可解释的滚动位置；当前条目消失时显示空状态而不是旧内容。
- [x] 使用 revision 和查询序号丢弃过期返回，覆盖快速搜索、快速切换筛选、连续收藏和拖放期间刷新。
- [x] 拖入悬浮球时球体和面板都显示明确拖入态；记录中禁止重复提交同一路径，完成后只显示结果反馈并刷新文件库。
- [x] 反馈信息区分新增、刷新、跳过、达到上限、失败和路径失效，不把“最近记录”重新渲染成主列表。
- [x] 对大索引使用分页或增量加载，搜索输入不触发无节制的全量 command 调用。
- [x] 面板打开和关闭时不产生明显跳动；几何尺寸改变不能导致球体锚点漂移。
- [x] 检查左、右、上、下边缘以及四角的面板方向、工作区约束、阴影和点击区域。
- [x] 检查不同 DPI 和多显示器切换下球体位置、面板尺寸、徽标和成功/失败标记。
- [x] 关闭主窗口、退出应用、隐藏到托盘、销毁悬浮球时清理监听器、定时器、未完成查询和预览资源引用。
- [x] 使用 `prefers-reduced-motion` 检查动画降级和状态文本仍然完整。

### 10.4 验证

- [ ] 主窗口新增文件、移除索引、收藏、重命名和重新定位后，悬浮窗列表最终一致。
- [x] 连续输入搜索词并快速清空时，最终列表只对应最后一次查询。
- [x] 连续拖入相同和不同路径时，索引写入不重复，文件库刷新不丢条目。
- [x] 在面板打开、关闭、拖动、边缘吸附、拖入和文件打开之间切换，悬停状态机没有卡在 `opening`、`closing` 或 `dragging`。
- [x] 在小工作区中面板能压缩到安全最小值，列表仍可滚动，操作按钮不会被裁切。
- [x] 运行 `npm.cmd run test:floating-ball`、`npm.cmd run test:library`、`npm.cmd run test:contracts` 和 `npm.cmd run build`。
- [x] Rust 侧运行 `cargo fmt --check`、`cargo check`、`cargo test` 和必要的 `cargo clippy --all-targets --all-features -- -D warnings`。
- [x] 使用 Playwright 或等价浏览器检查记录桌面/移动视口截图和核心交互；截图只使用测试演示数据。

### 10.5 阶段版本门禁

- [x] 同步五个版本入口到 `0.3.31`。
- [x] 在 `PROJECT_PROGRESS.md` 记录并发、同步、窗口几何、拖放和性能验证结果。
- [x] 明确列出仍需用户在 Windows 11 桌面端验证的项目，不以浏览器结果代替原生验收。
- [x] 提交阶段变更后进入阶段 F。
...
完成端到端回归、文档同步、候选版本验证和 Windows 11 原生验收，形成并发布 `v0.3.32`；本轮发布动作按用户授权执行。
...
- [x] `README.md` 和 `prototype/README.md` 将悬浮球主功能描述为“文件库”，明确写出文件列表、搜索、筛选、文件定位、文件夹目录、资源管理器显示和直接预览。
- [x] 文档单独说明“最近记录”仍属于保留的历史/托盘语义，不再是悬浮球弹窗主列表。
- [x] 文档区分自动化验证、浏览器回退检查和 Windows 11/Tauri/WebView2 原生验收。
- [x] 文档说明支持预览格式、失效路径、外部程序和 LibreOffice 等既有依赖边界，不扩大“已支持”声明。
- [x] `PROJECT_PROGRESS.md` 记录每个阶段的版本、主要文件、验证命令、候选安装包和用户验收结果。
- [x] 本轮未新增 IPC command、权限或数据结构；既有接口兼容性和安全边界已在阶段 A-E 记录。

### 11.4 自动验证清单

以下自动化证据沿用阶段 A-E 及此前阶段的已记录结果；本次仅进行文档收口和版本入口同步，未重复执行完整回归。

- [x] 运行 `npm.cmd run test:floating-ball`。
- [x] 运行 `npm.cmd run test:library`。
- [x] 运行 `npm.cmd run test:contracts`。
- [x] 运行 `npm.cmd run test:preview`。
- [x] 运行 `npm.cmd run test:settings`、`npm.cmd run test:operations` 和必要的 `npm.cmd run test:content`。
- [x] 运行 `npm.cmd run build`。
- [x] 在 `prototype/src-tauri` 运行 `cargo fmt --check`、`cargo check`、`cargo check --tests`、`cargo test`。
- [x] 适用时运行 `cargo clippy --all-targets --all-features -- -D warnings`。
- [x] 运行 `git diff --check`，确认版本入口、文档和代码没有编码或空白错误。

### 11.5 Windows 11 原生验收清单

以下项目必须在真实 Windows 11/Tauri 环境中验证，不能用浏览器回退模式替代：

- [x] 应用启动后悬浮球显示、置顶、透明背景和位置恢复正常。
- [x] 悬停展开、点击展开、从球体移动到面板、延迟收起和 `Escape` 行为正常。
- [x] 球体拖拽、边缘吸附、四角展开、面板方向和多显示器切换正常。
- [x] 混合 DPI 下球体大小、面板尺寸、数量徽标、成功/失败标记和点击区域正确。
- [x] 从资源管理器拖入文件和文件夹，记录结果、跳过原因和文件库刷新正确。
- [x] 文件库展示超过五项资料；搜索、筛选、排序和滚动不丢失条目。
- [x] 点击文件能够打开主窗口并定位到该文件。
- [x] 点击文件夹能够打开主窗口对应目录。
- [x] “在资源管理器中显示”能够定位文件和文件夹；失效路径显示正确错误。
- [x] 直接预览至少覆盖一个文本/Markdown、一个图片、一个 Office 文件和一个 PDF 流程；失败状态可恢复。
- [x] 主窗口导入、收藏、移除、重命名、重新定位后悬浮窗最终同步。
- [x] 隐藏到托盘、显示/隐藏悬浮球、主窗口关闭和真正退出时生命周期清理正常。
- [x] 重启应用后文件库、收藏、失效状态和悬浮球位置符合持久化约定。

用户于 2026-09-03 确认以上 Windows 11/Tauri/WebView2 原生验收清单全部通过；未提供具体 Windows、WebView2、显示器或 DPI 信息，文档不臆填。

### 11.6 候选版本门禁

- [x] 同步五个版本入口到 `0.3.32`，确认所有版本一致。
- [x] 已对 `0.3.32` 候选运行 `tauri:build` 和 `verify:loader`；NSIS 安装包为 `prototype/src-tauri/target/release/bundle/nsis/本地资料工作台_0.3.32_x64-setup.exe`，大小 `8806464` bytes，SHA-256 为 `40465EAA81D238576EA71DD286A30C6592B9D482E88EF7EBD83BF7FDBD268355`。
- [x] 用户已完成阶段 F 原生验收；安装、首次启动、托盘、悬浮球、重启和卸载行为与计划边界分别记录。
- [x] 已按用户明确授权推送 `dev`/`main`、创建 `v0.3.32` tag 并发布 GitHub Release；发布后不再修改文档。
- [x] 用户已确认 Windows 11 原生验收，`v0.3.32` 已标记为正式发布并发布 GitHub Release。
...
- [x] 旧的“最近记录”主面板计划和主语义已被本计划替换；当前 `PROJECT_PLAN.md` 只保留文件库改造路线。
- [x] `0.3.27` 至 `0.3.32` 每个阶段都有明确目标、文件边界、实现清单、验证方式和版本门禁。
- [x] 悬浮球外观升级已完成：图标、数量徽标、状态颜色、拖入反馈、成功/失败提示、动画降级和焦点样式均可验证。
- [x] 悬浮窗已显示全部文件库条目，支持搜索、筛选、排序、分页、空状态、错误重试和失效状态。
- [x] 点击文件直接打开主窗口并定位到该文件。
- [x] 点击文件夹打开主窗口对应目录。
- [x] 文件和文件夹都提供“在资源管理器中显示”快捷操作。
- [x] 支持格式提供直接预览入口，预览失败有明确下一步并释放资源。
- [x] 拖放记录、收藏、索引变更、主窗口操作和悬浮窗列表保持 revision 一致。
- [x] 自动化验证与 Windows 11/Tauri/WebView2 原生验收分别记录，未完成的原生验收不被描述为已完成。
- [x] 所有版本入口（当前为 `0.3.32`）、项目文档、进度文档和 Git 状态一致，未提交真实资料、构建缓存、密钥或临时产物；`v0.3.32` Tag/Release 已发布。
