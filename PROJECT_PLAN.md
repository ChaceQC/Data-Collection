...
- 已删除旧版总体计划内容并建立本文件。
- 阶段 A `0.3.17`、阶段 B `0.3.18` 和阶段 C `0.3.19` 已完成代码候选、最小自动验证和浏览器回退检查。

当前未完成：

- 阶段 D-J 尚未实现；当前版本入口为阶段 C 的 `0.3.19` 代码候选，正式发布基线仍为 `v0.3.16`。
- 阶段 A-C 的 Windows 11/Tauri/WebView2 原生窗口、真实文件预览、外部操作和多 DPI 验收仍需单独执行。
- `0.3.19` 已构建本地 Windows x64 NSIS 安装包，但尚未创建 Tag 或 Release；它表示阶段 C 代码候选，不代表正式发布。

下一步：

1. 安装并验证 `0.3.19` NSIS 候选，单独记录 Windows 11/Tauri/WebView2 真实预览和外部操作结果。
2. 根据具体桌面回归修复，或按计划进入阶段 D 的单条标签、分组编辑和资料详情。
