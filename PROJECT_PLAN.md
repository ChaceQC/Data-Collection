...
> 当前状态：阶段 A-I 的代码实现和阶段级自动验证已完成，尚未完成对应的 Windows 11 手工验收和候选版本提升
...
当前仍以 `0.3.6` 为已发布基线，阶段 A-I 的代码实现和阶段级自动验证已完成；阶段 G-I 的 Windows 11 手工验收尚未完成，下一步等待用户按阶段复核位置/筛选、标签/分组、批量/撤销及既有桌面能力，再决定是否将候选版本入口依次提升到 `0.3.13`-`0.3.15`。在用户验收和明确发布授权前，不修改应用实际版本，不执行发布流程，不把自动验证写成桌面验收。
