...
- [x] 使用现有预览注册表、测试夹具和 Rust 预览适配器检查已登记格式的成功/失败边界；真实 Windows 文件检查单独保留在下一项。
...
- [ ] Windows 11/Tauri 下重启后的标签持久化、真实分组编辑和分组删除结果仍需用户手工验收。
...
- [ ] Windows 11/Tauri/WebView2 下的真实文件导入、重启恢复、长路径和大规模性能仍需用户验收；本阶段不把浏览器回退或开发机自动测试写成原生验收。
...
- 已以 dev 分支的阶段 H `0.3.24` 代码候选为基线建立阶段 I 功能分支；正式发布基线为 `0.3.16`。
- 已删除旧版总体计划内容并建立本文件。
- 阶段 A `0.3.17`、阶段 B `0.3.18`、阶段 C `0.3.19`、阶段 D `0.3.20`、阶段 E `0.3.21` 和阶段 F `0.3.22` 已完成代码候选、最小自动验证和浏览器回退检查。
- 阶段 G `0.3.23` 和阶段 H `0.3.24` 已完成代码候选、最小自动验证、文档同步和 NSIS 候选构建；Windows 11/Tauri/WebView2 原生验收仍单独记录。
- 阶段 I `0.3.25` 已完成递归扫描、导入策略、进度、取消、超时、上限、安全校验、自动验证、文档同步和 NSIS 候选构建；Windows 11/Tauri/WebView2 原生验收仍单独记录。
- 阶段 J `0.3.26` 已完成纯文本/Markdown 正文索引、正则搜索、摘要高亮、增量同步、重建/清除、取消、损坏隔离、IPC/权限、自动验证、文档同步和 NSIS 候选构建；Windows 11/Tauri/WebView2 原生验收仍单独记录。

当前未完成：

- 阶段 J 的 Windows 11/Tauri/WebView2 原生验收、真实纯文本样本和大规模性能验收尚未由本代理代替用户完成；当前版本入口为 `0.3.26` 代码候选，正式发布基线仍为 `v0.3.16`。
- 阶段 A-I 的 Windows 11/Tauri/WebView2 原生窗口、真实文件预览、外部操作、多 DPI、元数据持久化、操作历史、设置冲突、最近打开、DOCX 后台转换和递归导入验收仍需单独执行。
- 阶段 D、E、F、G、H 和 I 的 Windows x64 NSIS 候选均只作为未发布候选，尚未创建新的 Tag 或 Release。

下一步：

1. 由用户安装并核对 `0.3.26` NSIS 候选，单独记录 Windows 11/Tauri/WebView2 下纯文本/Markdown 导入、正文检索、正则、重启恢复、清除/重建、取消和长路径结果。
2. 收到桌面验收结果后只修复具体阻断项；未获得明确授权前不创建新的 Tag、Release 或推送远程。
