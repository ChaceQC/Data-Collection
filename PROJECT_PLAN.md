...
> 计划状态：执行中
> 编制日期：2026-09-01
> 当前工作分支：codex/phase-i-recursive-import
> 当前发布基线：0.3.16
> 当前代码候选：0.3.25
> 下一阶段版本：0.3.26
...
- 阶段 J 尚未实现；当前版本入口为阶段 I 的 `0.3.25` 代码候选，正式发布基线仍为 `v0.3.16`。
...
1. 由用户安装并核对 `0.3.25` NSIS 候选，单独记录 Windows 11/Tauri/WebView2 下阶段 I 的文件夹选择、默认登记、递归策略、进度、取消、重启恢复和长路径结果。
2. 收到桌面验收结果后只修复具体阻断项；阶段 I 无阻断后再按计划进入阶段 J 的本地全文检索设计和实现。
