...
- 0.3.26 是本轮规划的最后一个阶段版本；阶段 A-J 已完成代码实现、自动验证和用户 Windows 11/Tauri/WebView2 原生验收。该完成状态不代表自动创建 Tag 或 GitHub Release，后续仍可在 0.3.x 范围内增加计划外修复阶段。
...
阶段 J 的索引方案、依赖、安全边界、增量更新、失败恢复、测试、文档和用户验收均已通过，版本已同步到 0.3.26。该版本仍未创建新的 Tag 或 Release，不把“计划完成”写成 Release 已发布。
...
- 当前代码候选为 `0.3.26`，正式发布基线仍为 `v0.3.16`；本轮没有创建新的 Tag、GitHub Release 或推送远程。
