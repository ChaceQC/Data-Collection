...
> 计划状态：阶段 E 代码候选已完成，Windows 原生验收待用户
> 编制日期：2026-09-02
> 当前工作分支：dev
> 当前发布基线：v0.3.26
> 计划候选版本：v0.3.31 - v0.3.32
...
| B | 0.3.28 | 悬浮球外观和面板骨架 | 球体状态视觉、数量徽标、文件库标题、面板尺寸和稳定布局 | 代码、自动验证和 NSIS 候选已完成；Windows 原生验收待用户 |
| C | 0.3.29 | 文件列表交互 | 全量列表、搜索、筛选、排序、分页、加载和空状态 | 代码、自动验证和 NSIS 候选已完成；Windows 原生验收待用户 |
| D | 0.3.30 | 主窗口连续工作流 | 文件定位、文件夹目录、资源管理器显示和直接预览 | 代码、自动验证和浏览器回退已完成；Windows 原生验收待用户 |
| E | 0.3.31 | 同步和稳定性 | 拖放反馈、revision 同步、竞态、混合 DPI、窄窗口和性能 | 代码、自动验证、浏览器回退和 NSIS 候选已完成；Windows 原生验收待用户 |
