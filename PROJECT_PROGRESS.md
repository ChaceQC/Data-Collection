...
### 阶段 F：预览生命周期和状态闭环（0.3.38 代码候选，自动门禁、NSIS 已完成）
...
- 无；阶段 F 代码、针对性测试、版本同步、NSIS 候选和 loader 校验已完成，待用户在 Windows 11/Tauri/WebView2 环境执行原生验收。

#### 用户验收

- 尚未执行。本阶段不能用浏览器回退、Rust/前端测试、生产构建或 NSIS 生成替代真实图片、PDF、Office、DOC、文本和 Markdown 的 WebView2 预览，以及快速切换、退出重启和失败夹具验收。
...
- `0.3.38` 只是本地候选，尚未创建 Tag、推送分支或创建 GitHub Release。

#### 下一步

- 用户使用 `prototype/tests/fixtures/preview/` 和无敏感 Windows 样本验收各格式真实预览、Range 分页、缺失/损坏/加密/超大/权限拒绝、LibreOffice 缺失、快速切换、退出重启和临时目录清理；通过后再单独决定是否发布。
