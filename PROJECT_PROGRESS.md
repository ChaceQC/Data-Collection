...
### 阶段 C：正文索引、摘要与搜索执行（0.3.43，安装包完成，待本地合并）

#### 已完成

- 从干净的本地 `dev` 创建 `codex/phase-0.3.43-content-search`，实现当前计划 R02、R11 及正文查询性能与顺序要求。
- 源字节、UTF-8 字节、Unicode 字符、总正文、条目和 JSON 共用边界；80 万汉字 GB18030 解码超限时只跳过当前项。旧超限缓存受控备份后隔离并保留有效正文，v1 格式不变。
- 正文快照与文档通过 Arc 共享，查询在受限 blocking worker 执行，最多一个扫描和一个等待任务，支持取消和协作式超时。前端至多一个在途和一个最新待执行请求，通过 requestId/cacheRevision/sourceRevision 丢弃旧结果。
- 增量同步先扣除已删除、失效和待替换正文；清除、重建废弃旧代次任务，取消或写入失败保留此前可用快照。清除后在新资料变更、显式重建或重启时恢复自动更新。
- 摘要按实际三字符前缀计算 Unicode 范围；共享夹具覆盖中文、非 BMP、多命中与截断，后端真实序列化和前端真实组件渲染对应验证。
- 五个版本文件统一为 `0.3.43`；同步 IPC、主窗口取消命令权限与生成 ACL、README 和 `docs/content-index.md`。正文存储按状态、同步、持久化、规则、查询与类型拆分，以控制本阶段模块体量。
...
- Edge 在 1280x800、720x800 检查正文设置和合成摘要，真实 `SearchHitSummary` 的 `<mark>` 内容为“研究😀”，页面及设置无横向溢出；保留原有单行摘要省略布局。截图在忽略的 `prototype/output/playwright/phase-c-*.png`，可通过 `tests/browser/content-search.js` 复现。
- 最终 `npm.cmd run tauri:build` 通过，包含生产前端、Rust release、Windows x64 NSIS 和 loader 校验；Sites 三个构建入口正常生成，未重复运行相同源码的单独前端 build。
- 打包复核补充调整重建入口顺序：先废弃旧代次任务，再获取当前主索引快照，避免在两步之间到达的新资料事件被清除且漏入重建。最终候选重新打包，不复用首轮源码的安装包。

#### 阻塞、风险与下一步

- 暂无开发阻塞；阶段 C 原生验收待用户执行，不以自动测试、Edge 或本地构建代替。协作式查询截止不能强制中断单次 regex 调用；未取消的单次调用仍受表达式与单文档上限约束。
- 当前只剩本地 Git 收口：交付记录提交后，通过 ancestry/ff-only 合入本地 `dev` 并安全删除阶段分支。
...
- 阶段起点 `dev` 为 `008ca291baba261db2f446682a5475f3a1feb680`；`main` 保持 `03298ae613284b70254db27555a87761ff6036e3`。独立阶段分支的实现提交为 `d59471d` 和 `e8b08f1`，等待文档提交后完成本地快进合并。
- Edge 与 Vite 服务均已关闭，临时端口 `49343` 已确认释放。安装包仍未签名、不内置 WebView2 Runtime，DOC 仍依赖可选 LibreOffice；既有 `xlsx@0.18.5` 风险不变。

#### 涉及模块

- Rust 正文 storage/command、app-data 上限、主窗口 capability/ACL；前端正文控制器、Repository、摘要、设置与 IPC 契约。
- 共享摘要夹具、正文 Rust/Hook/渲染回归、五个版本文件及交付文档。

## 2026-09-05

### 阶段 B：文件操作一致性与删除恢复（0.3.42，开发侧收口及用户原生验收已完成）

#### 已完成

- 从本地 `dev` 的 `99837ee` 创建独立分支 `codex/phase-0.3.42-file-recovery`，实现当前计划 R03-R05。
- 2026-09-05，用户明确回复“已验收”，阶段 B 原生验收登记为完成。用户未提供具体 Windows/WebView2 版本、显示器或样本明细，本文不补填这些字段。
...
- 用户验收登记仅更新计划、进度与说明文档，执行 `git diff --check` 和状态一致性检查；按用户减少测试的要求不重复测试、不重建安装包。
...
- 阶段 B 无开发侧实现、验证或打包待办。源码与安装包证据 `77b2f76` 已从独立分支 `codex/phase-0.3.42-file-recovery` 通过 ancestry 检查并以 `git merge --ff-only` 合入本地 `dev`；合并前 `dev` 为 `99837ee3e781cd07eac013b5048d2282afd9a132`。收口文档沿同一阶段分支同步后安全删除该分支，`main` 保持 `03298ae`。
- 阶段 B 无原生验收待办，用户已确认验收完成；验收状态通过独立文档分支 `codex/phase-0.3.42-acceptance` 同步到本地 `dev`，安装包对应源码仍为 `2cac223`。
- 下一代码阶段为 C `0.3.43`：按计划处理正文编码与大小边界、摘要高亮坐标和异步搜索；本次未开始阶段 C，未执行 push、Tag、Release 或上传。

#### 阻塞与风险

- 阶段 B 原生验收已由用户确认完成；自动回归和浏览器布局检查仍与原生验收分别记录。
...
- 根据本轮代码复查，完整替换 `PROJECT_PLAN.md`，删除旧执行路线；历史实现、发布和用户验收记录保留在本进度与 Git 历史中。
- 将 11 项缺陷和 3 类架构问题合并为七阶段：A 预览与异步状态 `0.3.41`、B 文件操作与恢复 `0.3.42`、C 正文索引与搜索 `0.3.43`、D 契约与自动门禁 `0.3.44`、E 前端职责迁移 `0.3.45`、F Rust 职责与性能 `0.3.46`、G 最终集成验收 `0.3.47`。
- 各阶段均明确五个版本文件同步、本地 Windows x64 NSIS 构建、loader 校验、安装包路径 / 大小 / SHA-256、定向验证和本地 Git 收口要求；本地打包失败时不得进入下一阶段。
- 契约、类型和 CI 安排在大范围重构之前；删除日志中间状态、正文编码边界和预览结果回写均有明确实现及回归要求。
- 按用户补充要求，明确每阶段必须在独立分支开发，通过代码、验证和本地安装包门禁后先并入 `dev`，下一阶段从更新后的 `dev` 开始；未完成合并不得进入下一阶段。

#### 本次文档提交范围

- 用户已明确要求将当前计划同步提交到本地 `main` 和 `dev`；本次将 `PROJECT_PLAN.md` 与配套进度作为同一文档提交同步，提交标识以 Git 记录为准。
- 本次文档同步不包含阶段代码实现、应用版本提升、远程 push、Tag 或 Release。

#### 进行中

- 阶段 A-G 均未开始代码实现，当前应用版本仍为 `0.3.40`。

#### 阻塞与风险

- 此次复查确认的功能缺陷与自动门禁缺口仍待修复，计划文字不代表修复完成。
- 历史 `0.3.40` 用户原生验收记录保留；新增路线的自动验证、本地安装包和用户原生验收须分别取证。

#### 下一步

- 按新计划从阶段 A `0.3.41` 开始，先保留大 PDF 读取失败回归，再修复目录上下文、在途导航、有限重试与文件级预览结果回写，完成后构建本地 NSIS 候选。
...
- 新计划通过 UTF-8 和 Markdown 结构检查：七阶段、连续版本 `0.3.41` 至 `0.3.47`、14 项缺陷 / 架构归属和七处阶段安装包门禁齐全，所有阶段初始状态为未开始。
- 本次仅修改规划与进度文档，不运行应用测试或安装包构建，不提前提升应用版本。
...
- 按当前 `AGENT.md` 和 `PROJECT_PLAN.md` 在独立分支 `codex/phase-0.3.38-preview-lifecycle` 实现 F1-F4；保持 `index.json` v5、`content-index.json` v1、`settings.json` v3、操作历史 v1 和悬浮球位置 v1 格式不变，不需要数据迁移。
- 预览资源在注册时保存源文件 metadata 快照；每次协议请求会重新执行普通文件、canonical path、大小和文件快照校验，读取前后再次复核，文件被替换或内容快照变化时返回冲突并释放资源。OPTIONS 只对已登记的随机 `previewId` 响应，PDF/视频大文件的首个无范围请求限制为 1 MiB 并返回完整 `Content-Range`。
- 新增 `record_preview_outcome` storage/IPC mutation，以当前 index revision 做乐观校验；状态不变时不写入、不递增 revision，`loading` 不作为最终状态持久化，启动时中断的 `loading` 归一为 `idle`。实际预览器成功渲染后在一次 mutation 中写入 `ready` 与 `lastOpenedAt`，失败、取消和超时保留对应终态。
- PreviewPane 继续为每个会话维护 request sequence、task ID 和 preview resource ID；关闭、切换、重试、卸载和应用退出均取消任务、终止 Worker、释放资源并清理 DOC 临时目录。PDF 加入文档和页面绘制超时，视频加入元数据超时，DOCX/XLSX fetch 在取消后不再向已终止 Worker 投递数据。
- 同步 Rust handler、build manifest、主窗口 capability、generated ACL/schema、前端 command 白名单、运行时 validator 和 TypeScript 声明；预览资源契约拒绝任意 URL/query、无效资源 ID、类型不一致和大小不一致结果。DOC 转换的无签名输出改为解析失败，不再误报为文件过大。

#### 进行中

- 无；阶段 F 代码、针对性测试、版本同步、NSIS 候选、loader 校验和用户原生验收均已完成。

#### 用户验收

- 2026-09-05，用户确认阶段 F 的 Windows 11/Tauri/WebView2 原生验收已完成。用户未提供具体 Windows、WebView2、显示器或样本明细，本文不补填这些环境字段。

#### 阻塞与风险

- 安装包未签名且不内置 WebView2 Runtime，目标 Windows 11 机器需要已有 WebView2；DOC 预览仍依赖目标机 LibreOffice，视频容器/编码仍取决于 WebView2。
- `xlsx@0.18.5` 的既有 Prototype Pollution/ReDoS 审计风险仍保留；本阶段继续使用 Worker、容器上限、行列/单元格上限和超时降低影响，没有替换依赖。
- `0.3.38` 仍是本地候选，尚未创建 Tag、推送分支或创建 GitHub Release；发布动作需要单独明确授权。

#### 下一步

- 当前没有阶段 F 的实现待办；如需发布 `0.3.38`，下一步是取得单独授权后执行 Tag、远程同步和 Release 流程，发布前继续保持本地候选边界。
...
- `prototype/src-tauri/build.rs`
- `prototype/src-tauri/capabilities/default.json`
- `prototype/src-tauri/permissions/autogenerated/record_preview_outcome.toml`
...
- `npm.cmd run test:preview`：16 项通过；JPG、文本/Markdown、DOCX、XLS/XLSX、PDF、视频注册和既有安全边界回归通过。
- `npm.cmd run test:contracts`：19 项通过；新增 preview revision、受控资源 URL/ID、资源大小和状态一致性校验通过。
- `npm.cmd run test:library`：26 项通过。
- `npm.cmd run build`：通过，生成 `dist/client/index.html`、`dist/server/index.js` 和 `dist/.openai/hosting.json`。
- `cargo fmt --all -- --check`、`cargo check --locked`、`cargo check --tests --locked`、`cargo test --locked`：均通过；Rust 共 107 项测试通过。
- `cargo clippy --locked --all-targets --all-features -- -D warnings`：通过。
- 五个版本入口 `prototype/package.json`、`prototype/package-lock.json` 根包、Tauri 配置、`Cargo.toml` 和 `Cargo.lock` 根 package 均为 `0.3.38`；`git diff --check` 通过。
- `npm.cmd run tauri:build`：通过，生成 Windows x64 NSIS 候选 `E:\Project\test\prototype\src-tauri\target\release\bundle\nsis\本地资料工作台_0.3.38_x64-setup.exe`，大小 `8976269` bytes，SHA-256 `7E07D46F1D04615658CF2E143C64FA49D50C5F5388398A22C2768531766F97E5`；release 主程序 FileVersion/ProductVersion 为 `0.3.38`，大小 `36456021` bytes，SHA-256 `A9E897C2A504E100DA1A3330641435CFF40E6E2F909492234ACCA6BE1B25239A`。
- `npm.cmd run verify:loader`：通过；`E:\Project\test\prototype\src-tauri\target\release\WebView2Loader.dll` 大小 `160320` bytes，SHA-256 `8427B1FC58EC707813E5C0A51EB5D69397BB333250A7B891BE4D3B123F1E0F1C`，确认 Windows x64 且与 release 主程序同目录。
- 浏览器回退检查：使用临时端口 `49217` 启动 Vite，Edge 本地页面加载、资料行模态预览、“浏览器演示限制”提示和 Escape 返回列表均通过；未读取或上传真实文件，检查后已关闭标签和服务，端口已释放。

## 2026-09-05

### 阶段 E：app data 安全和单实例保护（0.3.37 代码候选，自动门禁、NSIS、本地合并和用户验收已完成）
...
- 无；阶段 E 代码、针对性测试、生产构建、版本同步、NSIS 候选构建和 loader 校验已完成；阶段提交 `b0e91cd` 已通过 ancestry 检查并使用 `git merge --ff-only` 本地合并到 `dev`，阶段分支已删除。

#### 用户验收

- 2026-09-05，用户确认已完成阶段 E 的 Windows 11/Tauri/WebView2 原生验收。本记录不补填用户未提供的具体 Windows、WebView2、显示器或测试场景信息。
...
- 阶段 E 的候选仍未创建 Tag 或 Release。
- 浏览器回退和 Rust 自动化证据继续与用户已确认的 Windows 11/Tauri/WebView2 原生验收分开记录。

#### 下一步

- 无；阶段 E 的代码、文档、版本入口、自动门禁、NSIS 候选、本地合并和用户原生验收均已完成。
- 如继续推进架构加固路线，从阶段 F `0.3.38` 开始；是否 push、Tag 或创建 GitHub Release 仍需单独授权。
...
- 桌面列表通过后端返回的命中 ID 过滤，只把当前分页可见行的命中范围交给高亮渲染；浏览器回退继续使用内存演示数据和原有 JavaScript 模型，不把浏览器正则路径当作桌面实现。
- 同步 Rust handler、build manifest、主窗口 capability、generated ACL/schema、JS command 白名单、运行时 validator 和 `.d.ts`；未改变 `index.json`、`settings.json`、`content-index.json` 或 operation history 格式。
- 五个版本入口已同步为 `0.3.35`，README、prototype README、PROJECT_PLAN 和本进度文档已记录搜索协议限制及浏览器/桌面边界。
...
- 尚未执行。本阶段不能用 Rust/前端测试、生产构建或 NSIS 生成替代 Windows 11/Tauri/WebView2 原生验收；正则安全、中文/全角、长路径、快速输入、目录搜索和 20,000 条索引仍需用户在桌面环境确认。

#### 阻塞与风险

- 安装包未签名且不内置 WebView2 Runtime，目标 Windows 11 机器需要已有 WebView2；DOC 预览仍依赖目标机 LibreOffice。
- 本阶段没有改变持久化格式和原文件操作边界；`xlsx@0.18.5` 的既有依赖风险继续按发布边界记录。
...
- `prototype/src-tauri/src/storage/metadata_search.rs`
- `prototype/src-tauri/src/storage/content_search.rs`、`prototype/src-tauri/src/storage/mod.rs`
- `prototype/src-tauri/src/commands/mod.rs`、`prototype/src-tauri/src/lib.rs`、`prototype/src-tauri/build.rs`、`prototype/src-tauri/capabilities/default.json`、`prototype/src-tauri/permissions/autogenerated/search_metadata.toml`
- `prototype/src/features/library/libraryModel.js`、`useMetadataSearchController.js`、`LibraryPanel.jsx`、`LibraryPanelParts.jsx`、`libraryRepository.js`、`useIndexController.js`
- `prototype/src/lib/ipcContracts.js`、`ipcContracts.d.ts`、`prototype/tests/library-model.test.mjs`、`prototype/tests/ipc-contracts.test.mjs`
- 五个版本入口、`README.md`、`prototype/README.md`、`PROJECT_PLAN.md`

#### 验证

- `npm.cmd run test:library`：26 项通过。
- `npm.cmd run test:contracts`：18 项通过。
- `npm.cmd run test:content`：5 项通过。
- `npm.cmd run build`：通过，生成 Sites 所需的 `dist/client/index.html`、`dist/server/index.js` 和 `dist/.openai/hosting.json`。
...
- 当前 Git：分支为 `dev`，HEAD 为本阶段最终进度文档提交，工作树干净，较 `origin/dev` 领先且未推送；本阶段未执行 push、Tag 或 Release。
