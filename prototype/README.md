...
当前开发候选为新计划阶段 A `0.3.41`，修复 R01、R06-R09。目录请求统一按根 ID、相对路径和请求序号失效，索引同步最多三次、重试间隔 250/750 ms，最终失败保留列表并提供手动刷新。用户已于 2026-09-05 确认阶段 A 的 Windows 11/Tauri/WebView2 原生验收完成，正式发布基线仍为 `v0.3.40`，`0.3.41` 保持未发布本地候选。
