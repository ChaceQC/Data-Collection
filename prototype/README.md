...
当前开发候选为新计划阶段 B `0.3.42`，修复 R03-R05：文件/目录重定位选择器与稳定 ID 校验、重命名期间的并发元数据保留、同一资料物理操作互斥、删除日志恢复和重建空索引确认。主索引仍为 v5，`pending-operations.json` 升级为 v2；旧日志保留备份，结果不确定时要求核对，不在启动时重做删除。持久化顺序及旧版回退限制见 [文件操作与恢复](../docs/file-operation-recovery.md)。阶段 A、B 均已由用户确认原生验收，阶段 B 确认日期为 2026-09-05；正式发布基线仍为 `v0.3.40`，`0.3.42` 为未发布本地候选。
