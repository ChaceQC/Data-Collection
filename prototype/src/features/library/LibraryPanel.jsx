...
function FileTypeIcon({ kind }) {
  const iconByKind = { doc: FileDoc, docx: FileDoc, pdf: FilePdf, xlsx: FileXls, image: FileImage, video: FileVideo, folder: FolderSimple, markdown: FileText, text: FileText, other: FileText };
...
export function LibraryPanel({
  ...
  const visibleFiles = useMemo(() => {
    const filtered = filterEntries(sourceEntries, { activeNav, query: searchQuery, directory: Boolean(directoryView) });
  ...
  const heading = directoryView ? (
    <nav id="recent-title" className="folder-breadcrumbs" aria-label="文件夹路径">
      <button type="button" className="breadcrumb-button" onClick={() => onOpenBreadcrumb(-1)}><ArrowLeft size={17} weight="regular" aria-hidden="true" /><span>资料库</span></button>
    ...
    <section className="recent-section" aria-labelledby="recent-title" data-tauri-drag-region="false">
      ...
      <div className="library-toolbar">
        <label className="search-field">
          ...
          <input aria-label="搜索资料" value={searchQuery} placeholder="搜索名称、类型或状态" onChange={(event) => onSearchQueryChange(event.target.value)} />
            ...
            <table className="file-table-grid">
              ...
              <thead>
                <tr className="file-row file-row-header">
                  <th scope="col">名称</th><th scope="col">类型</th><th scope="col">大小</th><th scope="col">状态</th><th scope="col">修改时间</th><th scope="col">操作</th>
                </tr>
              </thead>
              <tbody>
                {page.entries.map((file) => (
                  <tr className={`file-row ${selectedId === file.id ? "is-selected" : ""}`} key={file.id} tabIndex={0} aria-selected={selectedId === file.id} onClick={() => onRowClick(file)} onKeyDown={(event) => onRowKeyDown(event, file)}>
                    <th scope="row" data-label="名称"><div className="file-name-cell"><FileTypeIcon kind={file.kind} /><span className="file-name" title={file.name}>{file.name}</span></div></th>
      ...
      ) : <EmptyState icon={<MagnifyingGlass size={28} weight="regular" />} title={getEmptyTitle({ activeNav, directoryView, searchQuery })} description={getEmptyDescription({ activeNav, directoryView, searchQuery })} />}
    </section>
  );
}
...
function EmptyState({ icon, title, description }) {
  return <div className="empty-state">{icon}<strong>{title}</strong><span>{description}</span></div>;
}
