...
当前发布版本为 `v0.3.26`，上一正式发布基线为 `v0.3.16`。`v0.3.26` 已完成此前阶段 A-J 的代码实现、最小自动验证、浏览器回退检查和用户 Windows 11/Tauri/WebView2 原生验收，并已创建 Tag 和 GitHub Release。当前工作区的悬浮球“文件库”计划已推进到阶段 E，代码候选版本为 `0.3.31`，尚未创建发布 Tag 或 Release；阶段 A-D 的查询契约、面板骨架、列表交互和主窗口快捷工作流已作为基础。项目源码位于 [`prototype/`](prototype/)，根目录的计划和进度记录分别见 [`PROJECT_PLAN.md`](PROJECT_PLAN.md) 与 [`PROJECT_PROGRESS.md`](PROJECT_PROGRESS.md)。
...
2. 也可以在桌面端把文件或文件夹拖到悬浮球，资料会登记到索引并在悬浮球文件库面板中显示；当前 `0.3.31` 候选已提供全量列表、搜索、筛选、排序、分页、revision 同步、重复路径抑制以及文件定位、文件夹目录、资源管理器显示和直接预览入口。
