...
当前开发候选为阶段 A `0.3.41`：大 PDF 使用 PDF.js 官方 `PDFDataRangeTransport` 显式分段读取，目录导航与索引刷新共享请求失效规则，同步最多尝试三次。预览回写使用绑定资料来源和任务的 Rust 内存凭证，其他资料的收藏、标签、分组或导入不会使当前预览漏记；目标替换、重新定位、移除或任务过期仍会拒绝旧结果。主索引保持 v5，无需数据迁移。该候选的 Windows 11/Tauri/WebView2 原生验收待用户执行。
