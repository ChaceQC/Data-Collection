...
当前发布版本为 `v0.3.32`，上一正式发布版本为 `v0.3.26`，历史基线为 `v0.3.16`。阶段 C `0.3.35` 当前为未发布的代码候选：桌面元数据搜索通过 Rust 线性时间 regex 执行，输入有长度/编译限制，结果只返回不透明 ID、命中字段和受控范围；阶段 B `0.3.34` 的一致索引快照以及阶段 A `0.3.33` 的 JPG 预览、目录子项预览操作、悬浮球数量徽标和查询归一化也已保留。阶段 B/C 尚未完成 Windows 11/Tauri/WebView2 原生验收、Tag 或 Release。`v0.3.32` 已完成阶段 F 的代码候选验证、文档收口和用户 Windows 11/Tauri/WebView2 原生验收，并已创建 Tag 和 GitHub Release。项目源码位于 [`prototype/`](prototype/)，根目录的计划和进度记录分别见 [`PROJECT_PLAN.md`](PROJECT_PLAN.md) 与 [`PROJECT_PROGRESS.md`](PROJECT_PROGRESS.md)。
...
- 版本号在前端 package、Tauri 配置和 Rust crate 入口保持一致，当前发布版本为 `v0.3.32`，阶段 C 当前候选为 `v0.3.35`，上一正式发布版本为 `v0.3.26`，历史基线为 `v0.3.16`；此前阶段 A-J 及当前阶段 F 的 Windows 11/Tauri/WebView2 手工验收已由用户完成并记录，阶段 B/C 原生验收待执行。
