...
当前发布版本为 `v0.3.32`，上一正式发布版本为 `v0.3.26`，历史基线为 `v0.3.16`。阶段 E `0.3.37` 已完成代码、自动门禁、NSIS、本地合并和用户 Windows 11/Tauri/WebView2 原生验收，仍未创建新的 Tag 或 Release。当前阶段 F `0.3.38` 已完成预览生命周期代码、自动门禁、NSIS、本地合并和用户 Windows 11/Tauri/WebView2 原生验收，仍是未发布本地候选；浏览器自动化和本地安装包不替代原生验收。项目源码位于 [`prototype/`](prototype/)，根目录的计划和进度记录分别见 [`PROJECT_PLAN.md`](PROJECT_PLAN.md) 与 [`PROJECT_PROGRESS.md`](PROJECT_PROGRESS.md)。
...
- 版本号在前端 package、Tauri 配置和 Rust crate 入口保持一致，当前发布版本为 `v0.3.32`，阶段 E 已验收的候选为 `v0.3.37`，阶段 F 当前候选为 `v0.3.38`，上一正式发布版本为 `v0.3.26`，历史基线为 `v0.3.16`；阶段 B/C/D 的 Windows 11/Tauri/WebView2 原生验收仍按 `PROJECT_PROGRESS.md` 记录，阶段 F 原生验收已由用户确认完成。
