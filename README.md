...
当前发布版本为 `v0.3.26`，上一正式发布基线为 `v0.3.16`。`v0.3.26` 已完成此前阶段 A-J 的代码实现、最小自动验证、浏览器回退检查和用户 Windows 11/Tauri/WebView2 原生验收，并已创建 Tag 和 GitHub Release。当前工作区的悬浮球“文件库”计划已推进到阶段 D，代码候选版本为 `0.3.30`，尚未创建发布 Tag 或 Release；阶段 A-C 的查询契约、面板骨架和列表交互已作为基础。项目源码位于 [`prototype/`](prototype/)，根目录的计划和进度记录分别见 [`PROJECT_PLAN.md`](PROJECT_PLAN.md) 与 [`PROJECT_PROGRESS.md`](PROJECT_PROGRESS.md)。
...
- 悬浮球文件库的文件行可以把文件定位到主窗口、把文件夹打开为主窗口目录，并在桌面端明确点击后直接预览或在资源管理器中显示；这些入口使用不透明资料 ID，浏览器回退只显示演示限制。
...
2. 也可以在桌面端把文件或文件夹拖到悬浮球，资料会登记到索引并在悬浮球文件库面板中显示；当前 `0.3.30` 候选已提供全量列表、搜索、筛选、排序、分页以及文件定位、文件夹目录、资源管理器显示和直接预览入口。
