...
当前发布版本为 `v0.3.26`，上一正式发布基线为 `v0.3.16`。`v0.3.26` 已完成此前阶段 A-J 的代码实现、最小自动验证、浏览器回退检查和用户 Windows 11/Tauri/WebView2 原生验收，并已创建 Tag 和 GitHub Release。当前工作区的悬浮球“文件库”计划已推进到阶段 C，代码候选版本为 `0.3.29`，尚未创建发布 Tag 或 Release；阶段 A 的查询契约和阶段 B 的面板骨架已作为基础。项目源码位于 [`prototype/`](prototype/)，根目录的计划和进度记录分别见 [`PROJECT_PLAN.md`](PROJECT_PLAN.md) 与 [`PROJECT_PROGRESS.md`](PROJECT_PROGRESS.md)。
...
- 悬浮球面板使用统一的悬停状态机和工作区几何模型，覆盖球体到面板的连续命中、按水平位置向左/右展开、面板内部滚动和键盘收起；阶段 C 的默认面板约为 `360 x 420 DIP`，窄视口会安全压缩并保留内部滚动。
...
2. 也可以在桌面端把文件或文件夹拖到悬浮球，资料会登记到索引并在悬浮球文件库面板中显示；当前 `0.3.29` 候选已提供全量列表、搜索、筛选、排序和分页。
