...
当前开发候选为新计划阶段 C `0.3.43`：统一正文源字节、解码后 UTF-8、字符数、总容量及 JSON 边界，受控备份后隔离旧超限缓存；修正中文及非 BMP 摘要高亮坐标。正文查询使用共享快照和受限异步 worker，清除、重建和快速输入废弃旧任务，写入失败保留可用缓存。格式保持 v1，限制、生命周期及旧版回退说明见 [正文索引与查询](docs/content-index.md)。阶段 C 的本地 NSIS/loader、Git 收口和用户原生验收均已完成，用户确认日期为 2026-09-05，`0.3.43` 仍为未发布本地候选，正式发布基线仍为 `v0.3.40`。
