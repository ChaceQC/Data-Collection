...
当前发布版本为 `v0.3.32`，上一正式发布版本为 `v0.3.26`，历史基线为 `v0.3.16`。阶段 E `0.3.37` 当前为未发布但已完成用户原生验收的代码候选：六类 app data 文件使用有界读取、受限备份、父目录/目标类型检查和原子写入，桌面端使用单实例转发并在外部文件操作前复核路径和文件身份；阶段 D `0.3.36` 的撤销差分、索引合并、重复路径去重、刷新映射和正文后台同步上限也已保留。阶段 B/C/D 尚未完成 Windows 11/Tauri/WebView2 原生验收、Tag 或 Release。`v0.3.32` 已完成阶段 F 的代码候选验证、文档收口和用户 Windows 11/Tauri/WebView2 原生验收，并已创建 Tag 和 GitHub Release。项目源码位于 [`prototype/`](prototype/)，根目录的计划和进度记录分别见 [`PROJECT_PLAN.md`](PROJECT_PLAN.md) 与 [`PROJECT_PROGRESS.md`](PROJECT_PROGRESS.md)。
...
导入、预览和正文索引都设置了大小、读取范围和解析限制。SVG、MOV、AVI、MKV 等未登记格式会显示为暂不支持。当前发布版本仍不提供云同步、在线账号或批量物理复制/重命名/删除；阶段 G 的最近打开和悬浮球连续工作流、阶段 H 的 DOCX 性能和取消、阶段 I 的递归导入和导入策略、阶段 J 的本地正文检索已完成代码实现并通过用户 Windows 11/Tauri 原生验收。正文索引只写入 Tauri app data 下独立的 `content-index.json`，不写入 `index.json`，也不把正文写入日志、toast、URL、操作历史或诊断导出。`v0.3.32` 已正式发布并创建 Tag/GitHub Release；旧悬浮球阶段 E/F 的 Windows 原生安装、真实拖放、混合 DPI、多显示器、资源管理器、文件预览和悬浮球生命周期验收均已由用户单独确认，当前架构加固阶段 B/C/D/E 的原生验收仍按本进度文档记录，浏览器回退和自动化结果仍按各自边界记录。
...
- 版本号在前端 package、Tauri 配置和 Rust crate 入口保持一致，当前发布版本为 `v0.3.32`，阶段 E 当前候选为 `v0.3.37`，上一正式发布版本为 `v0.3.26`，历史基线为 `v0.3.16`；此前阶段 A-J 及当前阶段 F 的 Windows 11/Tauri/WebView2 手工验收已由用户完成并记录，阶段 B/C/D 原生验收待执行，阶段 E 原生验收已由用户确认完成。
