...
当前开发候选为新计划阶段 C `0.3.43`：统一正文源字节、解码后 UTF-8、字符数、总容量及 JSON 边界，受控备份后隔离旧超限缓存；修正中文及非 BMP 摘要高亮坐标。正文查询使用共享快照和受限异步 worker，清除、重建和快速输入废弃旧任务，写入失败保留可用缓存。格式保持 v1，限制、生命周期及旧版回退说明见 [正文索引与查询](docs/content-index.md)。阶段 C 的本地 NSIS/loader 与 Git 收口已完成，原生验收待用户确认，正式发布基线仍为 `v0.3.40`。

上一候选阶段 B `0.3.42`：重定位按文件或文件夹选择目标，排序后按稳定 ID 返回；重命名保留提交时最新的收藏、标签、分组和历史字段，同一资料的物理操作互斥。删除日志升级为 v2，增加受控来源快照，启动和刷新时核对未完成操作；不确定的结果保留核对入口，重建空索引需确认影响范围并同步清理日志。主索引保持 v5，旧删除日志先备份再迁移，详见 [文件操作与恢复](docs/file-operation-recovery.md)。用户已于 2026-09-05 确认阶段 B 原生验收完成，`0.3.42` 仍为未发布本地候选。
