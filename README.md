...
当前发布版本为 `v0.3.32`，上一正式发布版本为 `v0.3.26`，历史基线为 `v0.3.16`。阶段 E `0.3.37` 当前为未发布的代码候选：六类 app data 文件使用有界读取、受限备份、父目录/目标类型检查和原子写入，桌面端使用单实例转发并在外部文件操作前复核路径和文件身份；阶段 D `0.3.36` 的撤销差分、索引合并、重复路径去重、刷新映射和正文后台同步上限也已保留。阶段 B/C/D/E 尚未完成 Windows 11/Tauri/WebView2 原生验收、Tag 或 Release。`v0.3.32` 已完成阶段 F 的代码候选验证、文档收口和用户 Windows 11/Tauri/WebView2 原生验收，并已创建 Tag 和 GitHub Release。项目源码位于 [`prototype/`](prototype/)，根目录的计划和进度记录分别见 [`PROJECT_PLAN.md`](PROJECT_PLAN.md) 与 [`PROJECT_PROGRESS.md`](PROJECT_PROGRESS.md)。
...
- 版本号在前端 package、Tauri 配置和 Rust crate 入口保持一致，当前发布版本为 `v0.3.32`，阶段 E 当前候选为 `v0.3.37`，上一正式发布版本为 `v0.3.26`，历史基线为 `v0.3.16`；此前阶段 A-J 及当前阶段 F 的 Windows 11/Tauri/WebView2 手工验收已由用户完成并记录，阶段 B/C/D/E 原生验收待执行。
