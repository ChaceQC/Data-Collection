...
当前开发候选为阶段 B `0.3.42`：重定位按文件或文件夹选择目标，排序后按稳定 ID 返回；重命名保留提交时最新的收藏、标签、分组和历史字段，同一资料的物理操作互斥。删除日志升级为 v2，增加受控来源快照，启动和刷新时核对未完成操作；不确定的结果保留核对入口，重建空索引需确认影响范围并同步清理日志。主索引保持 v5，旧删除日志先备份再迁移，详见 [文件操作与恢复](docs/file-operation-recovery.md)。`0.3.42` 为未发布本地候选，Windows 11/Tauri/WebView2 原生验收待用户完成。
