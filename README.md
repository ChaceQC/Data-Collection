...
当前发布版本为 `v0.3.26`，上一正式发布基线为 `v0.3.16`。本版本在 `0.3.16` 的资料库能力之上完成新计划阶段 A-J 的代码实现、最小自动验证、浏览器回退检查和用户 Windows 11/Tauri/WebView2 原生验收，并已创建 Tag 和 GitHub Release。项目源码位于 [`prototype/`](prototype/)，根目录的计划和进度记录分别见 [`PROJECT_PLAN.md`](PROJECT_PLAN.md) 与 [`PROJECT_PROGRESS.md`](PROJECT_PROGRESS.md)。
...
导入、预览和正文索引都设置了大小、读取范围和解析限制。SVG、MOV、AVI、MKV 等未登记格式会显示为暂不支持。当前发布版本仍不提供云同步、在线账号或批量物理复制/重命名/删除；阶段 G 的最近打开和悬浮球连续工作流、阶段 H 的 DOCX 性能与取消、阶段 I 的递归导入和导入策略、阶段 J 的本地正文检索已完成代码实现并通过用户 Windows 11/Tauri 原生验收。正文索引只写入 Tauri app data 下独立的 `content-index.json`，不写入 `index.json`，也不把正文写入日志、toast、URL、操作历史或诊断导出。`v0.3.26` 已正式发布。
...
- 版本号在前端 package、Tauri 配置和 Rust crate 入口保持一致，当前发布版本为 `v0.3.26`，上一正式发布版本为 `v0.3.16`；阶段 A-J 的 Windows 11/Tauri/WebView2 手工验收已由用户完成并记录。
