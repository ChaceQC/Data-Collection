...
当前发布版本为 `v0.3.43`，上一正式发布版本为 `v0.3.40`，历史基线为 `v0.3.16`。新计划阶段 C `0.3.43` 已完成代码、自动门禁、NSIS、本地合并和用户 Windows 11/Tauri/WebView2 原生验收，并已创建 Tag、发布 GitHub Release；浏览器自动化和本地安装包不替代原生验收。项目源码位于 [`prototype/`](prototype/)，根目录的计划和进度记录分别见 [`PROJECT_PLAN.md`](PROJECT_PLAN.md) 与 [`PROJECT_PROGRESS.md`](PROJECT_PROGRESS.md)。
...
当前发布版本为新计划阶段 C `0.3.43`：统一正文源字节、解码后 UTF-8、字符数、总容量及 JSON 边界，受控备份后隔离旧超限缓存；修正中文及非 BMP 摘要高亮坐标。正文查询使用共享快照和受限异步 worker，清除、重建和快速输入废弃旧任务，写入失败保留可用缓存。格式保持 v1，限制、生命周期及旧版回退说明见 [正文索引与查询](docs/content-index.md)。阶段 C 的本地 NSIS/loader、Git 收口和用户原生验收均已完成，用户确认日期为 2026-09-05，`v0.3.43` 已创建 Tag 并发布 GitHub Release，正式发布基线已更新为 `v0.3.43`。

此前候选阶段 B `0.3.42`：重定位按文件或文件夹选择目标，排序后按稳定 ID 返回；重命名保留提交时最新的收藏、标签、分组和历史字段，同一资料的物理操作互斥。删除日志升级为 v2，增加受控来源快照，启动和刷新时核对未完成操作；不确定的结果保留核对入口，重建空索引需确认影响范围并同步清理日志。主索引保持 v5，旧删除日志先备份再迁移，详见 [文件操作与恢复](docs/file-operation-recovery.md)。用户已于 2026-09-05 确认阶段 B 原生验收完成，`0.3.42` 未单独发布，当前发布版本为 `v0.3.43`。

阶段 A `0.3.41` 的大 PDF 分段读取、目录请求失效、有限索引同步重试和文件级预览凭证继续保留；用户已于 2026-09-05 确认阶段 A 原生验收完成。
...
- 预览资源绑定注册时的文件快照，在请求和实际读取前后复核路径、大小与文件身份；PDF/视频的显式 `Range` 单次最多读取 1 MiB，大文件无 Range 的 GET 明确拒绝，HEAD 返回真实总长度。`0.3.41` 使用文件级任务凭证回写，实际渲染成功才同时更新 `ready` 和 `lastOpenedAt`。
...
导入、预览和正文索引都设置了大小、读取范围和解析限制。SVG、MOV、AVI、MKV 等未登记格式会显示为暂不支持。当前发布版本仍不提供云同步、在线账号或批量物理复制/重命名/删除；阶段 G `0.3.39` 的模块拆分、结构化 IPC 错误、parity 门禁、本地 NSIS 候选、本地合并和 Windows 11/Tauri 原生验收均已完成，阶段 H `0.3.40` 已完成最终回归和用户原生验收并发布。正文索引只写入 Tauri app data 下独立的 `content-index.json`，不写入 `index.json`，也不把正文写入日志、toast、URL、操作历史或诊断导出。新计划阶段 C `0.3.43` 已正式发布并创建 Tag/GitHub Release；新计划阶段 A-C 的代码、验收和安装包状态以 `PROJECT_PROGRESS.md` 为准，当前没有阶段 C 之后的执行计划，浏览器回退和自动化结果仍按各自边界记录。
...
- 版本号在前端 package、Tauri 配置和 Rust crate 入口保持一致，当前发布版本为 `v0.3.43`，上一正式发布版本为 `v0.3.40`，历史基线为 `v0.3.16`；新计划阶段 A-C 的 Windows 11/Tauri/WebView2 原生验收、Tag 和 Release 状态按 `PROJECT_PROGRESS.md` 记录。
