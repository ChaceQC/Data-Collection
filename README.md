...
当前版本为 `0.3.16`，在 `0.3.6` 基础上合并完成新计划阶段 A-I 的代码实现、自动验证，以及阶段 J 的用户验收与发布门禁确认；用户已确认阶段 A-J 验收完成，本次对应发布为 `v0.3.16`。项目源码位于 [`prototype/`](prototype/)，根目录的计划和进度记录分别见 [`PROJECT_PLAN.md`](PROJECT_PLAN.md) 与 [`PROJECT_PROGRESS.md`](PROJECT_PROGRESS.md)。

当前 `0.3.16` 已包含新计划阶段 A-I 的代码和阶段级自动验证，并完成阶段 J 的用户验收与发布门禁确认，包括登记路径授权、索引 revision 刷新、索引恢复、文件操作待同步记录、可取消预览、共享 IPC 契约、语义资料表、位置搜索、标签/分组、批量索引操作和有限撤销。阶段 A-J 均已按用户确认完成验收。
...
导入和预览都设置了大小、读取范围和图片解码尺寸限制。SVG、MOV、AVI、MKV 等未登记格式会显示为暂不支持。当前版本仍不提供云同步、在线账号、全文检索或批量物理复制/重命名/删除；阶段 G-I 的位置、标签/分组、批量索引操作和有限撤销已随 `0.3.16` 发布。阶段 J 的用户验收与发布门禁确认也已完成。
...
- 版本号在前端 package、Tauri 配置和 Rust crate 入口保持一致，当前发布版本为 `0.3.16`；用户 Windows 11/Tauri/WebView2 阶段 A-J 手工验收已完成。
