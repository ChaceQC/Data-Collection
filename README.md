...
- 保存路径、显示名、类型、大小、修改时间、收藏状态和预览状态等索引信息。
- 按名称、类型和状态搜索，按添加时间、修改时间、名称或大小排序，并支持分页。
...
| PDF | `.pdf` | 支持分页和缩放；内容不会作为可信 HTML 执行。 |
| 旧版 Word 文档 | `.doc` | 通过本机 LibreOffice 转换为临时 PDF 后预览；缺少转换器时无法预览。 |

导入和预览都设置了大小、读取范围和图片解码尺寸限制。SVG、MOV、AVI、MKV 等未登记格式会显示为暂不支持。当前版本尚未实现云同步、在线账号、全文检索、标签/分组、批量操作和通用撤销栈。
...
- 原生层：Rust stable。
- 预览依赖：Marked、DOMPurify、Mammoth、SheetJS 和 PDF.js。
